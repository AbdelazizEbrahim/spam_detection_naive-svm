...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('naive_bayes.log'),
...
def clean_dataset(file_path, output_path='data/cleaned_dataset_nb.csv'):
...
def evaluate_model(model, X_test, y_test):
    ...
    log_time("Naïve Bayes evaluation", start)
...
def main():
    logger.info("Starting Naïve Bayes spam classification pipeline")
    try:
        ...
        nb_preds, nb_accuracy = evaluate_model(nb_model, X_test_vect, y_test)
        
        # Log top spam features
        feature_names = vectorizer.get_feature_names_out()
        spam_probs = np.exp(nb_model.feature_log_prob_[1])
        top_spam_features = sorted(zip(feature_names, spam_probs), key=lambda x: x[1], reverse=True)[:10]
        logger.info("Top spam features: %s", top_spam_features)
        ...
        while True:
            ...
            if user_input.lower() == 'exit':
                ...
                break
            pred, elapsed = classify_text(nb_model, vectorizer, user_input)
            logger.info(f"Prediction: {pred} (Time: {elapsed:.4f}s)")
            explain_prediction(nb_model, vectorizer, user_input)
